...
member.roles.remove(set.roller.vip)

message.channel.send(new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setColor('GREEN').setFooter(conf.status).setDescription(`${member} kullanıcıdan  ${set.roller.vip} rolü başarıyla alındı`)
