...
if(member.id === client.user.id) return message.react("❌").then(x => x.delete({timeout: 5000}));
...
if (db.fetch(`tag.${message.guild.id}`)) {
 if(!member.user.username.includes(set.taglar.tag) && !member.roles.cache.has(set.vip) && !member.roles.cache.has(set.roller.booster)) return message.channel.send(new MessageEmbed().setColor('RED').setDescription('Sunucumuz şuanda taglı alımdadır. Tagımızı alıp kayıt olabilirsiniz.').setFooter('Bu uygulama \`vip\` ve \`booster\` üyeler için geçerli değildir.'))
}
...
kdb.add(`nioche.${message.author.id}.erkek`, 1)
...
.setTimestamp()
.setDescription(`${member} kullanıcı <@&${set.roller.erkek}> olarak kayıt edildi.`)
